...
如果您的集群支持 LoadBalancer 服务，可以通过下面的命令在安装过程中就指定创建 LoadBalancer 类型的服务。
...
除了针对集群和租户的性能指标监控功能外，OceanBase Dashboard 提供了集群管理、租户管理、备份管理和终端连接等方便用户运维 OceanBase 集群的功能，并且 OBProxy 管理、监控告警、SQL 监控、集群诊断等功能也在紧密的开发过程中，会在 Dashboard 后续的版本中发布。
