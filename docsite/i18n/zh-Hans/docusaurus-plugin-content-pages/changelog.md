...
## 2.3.4 (发布于 2026.01.13)

功能优化
1. 支持通过环境变量设置 oceanbase-sdk 的超时时间。
2. 重启过程中，跳过处于恢复状态的 observer。
3. 优化了部分默认超时时间的设置。
4. 优化了获取 OceanBase 系统租户连接客户端的逻辑。

缺陷修复
1. 为新添加的 obzone 增加了 support-static-ip 注解。
2. 修复了在 observer 恢复过程中，若 pod 被删除会导致任务失败的问题。
3. 修复了当 obtenant 的 unit 态异常时导致的程序 panic 问题。
4. 增加了在 obzone 滚动更新期间对其 spec 变更的检查。
