...
It's recommended to install OceanBase Dashboard using the Kubernetes package manager Helm. After confirming that Helm has been installed, execute the following three commands to install OceanBase Dashboard in the default namespace.
...
If the installation command is executed successfully, the following notes will be displayed in the terminal.
...
If your cluster supports LoadBalancer services, you can specify the creation of a LoadBalancer type service during the installation process with the following command.

```bash
helm install oceanbase-dashboard ob-operator/oceanbase-dashboard --version=0.2.1 --set service.type=LoadBalancer
```
...
Besides the performance monitoring features for clusters and tenants, OceanBase Dashboard provides functions for cluster management, tenant management, backup management, and terminal connections to facilitate the operation and maintenance of OceanBase clusters. OBProxy management, monitoring alarms, SQL monitoring, and cluster diagnostics are also under active development and will be released in subsequent versions of the Dashboard.
