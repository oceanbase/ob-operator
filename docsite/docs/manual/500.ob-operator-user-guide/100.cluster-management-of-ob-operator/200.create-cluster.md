...
| `mode` | `standalone`: Bootstrap the single-node cluster with 127.0.0.1, which cannot contact other nodes any more. <br/> `service`: Create a specific K8s service for each OBServer and use the service's `ClusterIP` as the OBServer's IP address. |
