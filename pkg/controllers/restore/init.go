...
import (
	...
	"github.com/oceanbase/ob-operator/pkg/controllers/restore/core"
	"github.com/oceanbase/ob-operator/pkg/infrastructure/kube"
	"github.com/oceanbase/ob-operator/pkg/kubeclient"
)

var (
	controllerKind = cloudv1.SchemeGroupVersion.WithKind("Restore")
)
...
func newReconciler(mgr manager.Manager) reconcile.Reconciler {
	return &core.RestoreReconciler{
		CRClient: kubeclient.NewClientFromManager(mgr),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor(restoreconst.ControllerName),
	}
}
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	c, err := controller.New(
		restoreconst.ControllerName,
		mgr,
		controller.Options{
			Reconciler:              r,
			MaxConcurrentReconciles: restoreconst.ConcurrentReconciles,
		},
	)
	if err != nil {
		klog.Errorln(err)
	...
	err = c.Watch(
		&source.Kind{Type: &cloudv1.Restore{}},
