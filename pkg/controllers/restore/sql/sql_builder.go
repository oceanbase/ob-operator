...
func DoResotreSQLReplacer(dest_tenant, source_tenant, dest_path, time, backup_cluster_name, backup_cluster_id, pool_list string) *strings.Replacer {
	return strings.NewReplacer("${dest_tenant}", dest_tenant, "${source_tenant}", source_tenant, "${dest_path}", dest_path, "${time}", time, "${backup_cluster_name}", backup_cluster_name, "${backup_cluster_id}", backup_cluster_id, "${pool_list}", pool_list)
}
