...
func (p *backupEventHandler) Create(evt event.CreateEvent, q workqueue.RateLimitingInterface) {
	backup := evt.Object.(*cloudv1.Backup)
	if backup.DeletionTimestamp != nil {
		p.Delete(event.DeleteEvent{Object: evt.Object}, q)
		return
	}
	controllerRef := metav1.GetControllerOf(backup)
	if controllerRef != nil && isRefController(controllerRef) {
		p.enqueueHandler.Create(evt, q)
	}
}

func (p *backupEventHandler) Update(evt event.UpdateEvent, q workqueue.RateLimitingInterface) {
...
func (p *backupEventHandler) Delete(evt event.DeleteEvent, q workqueue.RateLimitingInterface) {
	backup := evt.Object.(*cloudv1.Backup)
	controllerRef := metav1.GetControllerOf(backup)
	if controllerRef != nil && isRefController(controllerRef) {
...
func (p *backupEventHandler) Generic(evt event.GenericEvent, q workqueue.RateLimitingInterface) {
}
