...
import (
	...
	"github.com/oceanbase/ob-operator/pkg/controllers/backup/core"
	"github.com/oceanbase/ob-operator/pkg/infrastructure/kube"
	"github.com/oceanbase/ob-operator/pkg/kubeclient"
)

var (
	controllerKind = cloudv1.SchemeGroupVersion.WithKind("Backup")
)
...
func newReconciler(mgr manager.Manager) reconcile.Reconciler {
	return &core.BackupReconciler{
		CRClient: kubeclient.NewClientFromManager(mgr),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor(backupconst.ControllerName),
	}
}
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	c, err := controller.New(
		backupconst.ControllerName,
		mgr,
		controller.Options{
			Reconciler:              r,
			MaxConcurrentReconciles: backupconst.ConcurrentReconciles,
		},
	)
	if err != nil {
		klog.Errorln(err)
	...
	err = c.Watch(
		&source.Kind{Type: &cloudv1.Backup{}},
