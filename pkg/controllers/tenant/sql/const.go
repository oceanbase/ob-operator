...
const (
	GetGvTenantListSQL     = "SELECT tenant_id, tenant_name, zone_list, primary_zone, collation_type, read_only, locality FROM oceanbase.gv$tenant"
	...
	CreateTenantSQLTemplate = "CREATE TENANT IF NOT EXISTS ${TENANT_NAME} CHARSET='${CHARSET}', ZONE_LIST=('${ZONE_LIST}'), PRIMARY_ZONE='${PRIMARY_ZONE}', RESOURCE_POOL_LIST=(${RESOURCE_POOL_LIST}) ${LOCALITY}${COMMENT}${DEFAULT_TABLEGROUP}${COLLATE}${LOGONLY_REPLICA_NUM} ${VARIABLE_LIST} "
	...
	SetTenantSQLTemplate         = "ALTER TENANT ${TENANT_NAME} ${ZONE_LIST} ${PRIMARY_ZONE} ${RESOURCE_POOL_LIST} ${CHARSET} ${DEFAULT_TABLEGROUP} ${LOGONLY_REPLICA_NUM}"
