...
cleanClusteroperation() {
    ...
    echo $template_file
    echo $OP_SCALE_RESOURCE
    kubectl delete obclusteroperation $template_file -n $NAMESPACE
}

prepare() {
    export OP_SCALE_RESOURCE=op-scale-resource-$SUFFIX
...
check_operation() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	obclusteroperation=`kubectl get obclusteroperation -n $NAMESPACE | grep $OP_SCALE_RESOURCE |awk -F' ' '{print $3}'| tail -n 1`
...
prepare
validate
#cleanClusteroperation "$OP_SCALE_RESOURCE"
echo $OP_SCALE_RESOURCE
