...
prepare() {
    export OP_DEL_MONITOR=op-del-monitor-$SUFFIX
...
check_operation() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	obclusteroperation=`kubectl get obclusteroperation -n $NAMESPACE | grep $OP_DEL_MONITOR |awk -F' ' '{print $3}'| tail -n 1`
...
prepare
validate
#cleanClusteroperation "$OP_DEL_MONITOR"
echo $OP_DEL_MONITOR
