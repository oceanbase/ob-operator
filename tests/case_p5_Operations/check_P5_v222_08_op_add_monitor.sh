...
prepare() {
    export OP_ADD_MONITOR=op-add-monitor-$SUFFIX
...
check_operation() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	obclusteroperation=`kubectl get obclusteroperation -n $NAMESPACE | grep $OP_ADD_MONITOR |awk -F' ' '{print $3}'| tail -n 1`
...
prepare
validate
#cleanClusteroperation "$OP_ADD_MONITOR"
