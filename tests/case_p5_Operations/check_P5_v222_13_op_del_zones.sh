...
prepare() {
    export OP_DEL_ZONE=op-delete-zones-$SUFFIX
    export OP_ADD_ZONE_TWO=op-add-zone-two-$SUFFIX
...
check_operation() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	obclusteroperation=`kubectl get obclusteroperation -n $NAMESPACE | grep $OP_DEL_ZONE |awk -F' ' '{print $3}'| tail -n 1`
...
check_operation_add_zone() {
    ...
    while true; do
        echo 'check obclusteroperation'
        counter=$((counter+1))
        obclusteroperation=`kubectl get obclusteroperation -n $NAMESPACE | grep $OP_ADD_ZONE_TWO |awk -F' ' '{print $3}'| tail -n 1`
        if [[ $obclusteroperation = "Succeeded"  ]]
        then
            echo "obclusteroperation $obclusteroperation"
            OPERATION_ACTIVE='true'
...
validate() {
    ...
    if [[ $RESOURCE_RUNNING == 'false' ]]; then
        ...
        check_in_obcluster
	create "op_del_zones.yaml"
 	check_operation
	check_resource_running
	create "op_add_zone2.yaml"
	check_operation_add_zone
