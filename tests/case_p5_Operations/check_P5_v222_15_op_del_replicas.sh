...
prepare() {
    export OP_DEL_REPLICAS=op-del-replicas-$SUFFIX
...
check_operation() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	obclusteroperation=`kubectl get obclusteroperation -n $NAMESPACE | grep $OP_DEL_REPLICAS |awk -F' ' '{print $3}'| tail -n 1`
