...
prepare() {
    export OP_RESTART_OBSERVERS=op-restart-observers-$SUFFIX
...
check_operation() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	obclusteroperation=`kubectl get obclusteroperation -n $NAMESPACE | grep $OP_RESTART_OBSERVERS |awk -F' ' '{print $3}'| tail -n 1`
...
validate() {
    ...
    if [[ $RESOURCE_RUNNING == 'false' ]]; then
        ...
        check_in_obcluster
	create "op_restart_observers.yaml"
 	check_operation
	check_resource_running
