...
prepare() {
    export OP_ADD_ZONE=op-add-zones-$SUFFIX
...
check_operation() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	obclusteroperation=`kubectl get obclusteroperation -n $NAMESPACE | grep $OP_ADD_ZONE |awk -F' ' '{print $3}'| tail -n 1`
