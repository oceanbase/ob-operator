...
prepare() {
    export OP_MODIFY_SC=op-modify-sc$SUFFIX 
...
check_operation() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	obclusteroperation=`kubectl get obclusteroperation -n $NAMESPACE | grep $OP_MODIFY_SC |awk -F' ' '{print $3}'| tail -n 1`
