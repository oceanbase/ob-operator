...
prepare() {
    export OP_DELETE_BACKUP_VOL=op-delete-backup-vol-$SUFFIX 
...
check_operation() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	obclusteroperation=`kubectl get obclusteroperation -n $NAMESPACE | grep $OP_DELETE_BACKUP_VOL |awk -F' ' '{print $3}'| tail -n 1`
