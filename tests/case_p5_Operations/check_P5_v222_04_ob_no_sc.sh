...
prepare() {
    ...
    export OBCLUSTER_NAME_NO_SC=testnosc$SUFFIX
...
check_resource_running() {
    ...
    while true; do
        echo 'check resource'
        counter=$((counter+1))
        pod_1_zone1=`kubectl get pod  -o wide -n $NAMESPACE | grep $OBCLUSTER_NAME_NO_SC-1-zone1 |awk -F' ' '{print $2}'| awk 'NR==1'`
        ip=`kubectl get pod  -o wide -n $NAMESPACE | grep $OBCLUSTER_NAME_NO_SC-1-zone1 |awk -F' ' '{print $6}'| awk 'NR==1'`
        crd_obcluster=`kubectl get obcluster $OBCLUSTER_NAME_NO_SC -n $NAMESPACE  -o yaml| grep "status: running" | tail -n 1| sed 's/ //g'`
