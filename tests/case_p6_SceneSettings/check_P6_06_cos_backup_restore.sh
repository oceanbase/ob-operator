...
prepare() {
    ...
    export COS_ACCESS=cos-access-$SUFFIX
    export OBTENANT_BP_COS=obtenant-bp-cos-$SUFFIX
    export COS_BACKUP_PATH=cos-backup-path-$SUFFIX
    export COS_ARCHIVE_PATH=cos-archive-path-$SUFFIX
    export OBTENANT_RESTORE_COS=obtenantrestorecos$SUFFIX
    ...
    create_pass_cos_secret $NAMESPACE $COS_ACCESS $COS_AK $COS_SK $COS_APPID 
    echo $PASSWORD
}
...
check_backup_running() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	crd_obtenantbackuppolicies_ARCHIVE=`kubectl get obtenantbackuppolicies.oceanbase.oceanbase.com -n $NAMESPACE| grep "$OBTENANT_BP_COS"|awk -F' ' '{print $2}'`
...
check_restore_running() {
	...
	counter=$((counter+1))
	crd_primary_restore=`kubectl get obtenant -n $NAMESPACE | grep "$OBTENANT_RESTORE_COS"|awk -F' ' '{print $2}'`
	crd_primary_obtenant=`kubectl get obtenantrestore -n $NAMESPACE| grep "$OBTENANT_RESTORE_COS"|awk -F' ' '{print $2}'`
...
check_restore_db_running() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	tenant_role=`mysql -h $ip -P2881 -A -uroot -p$PASSWORD -Doceanbase -e "select * from DBA_OB_TENANTS;"| grep $OBTENANT_RESTORE_COS|awk -F' ' '{print $17}'`
	echo $tenant_role
...
validate() {
	...
	check_backup_db_running
	sleep 100s
	run "obtenant-restore-cos.yaml"
	catfile "obtenant-restore-cos.yaml"
	check_restore_running
	check_restore_db_running
