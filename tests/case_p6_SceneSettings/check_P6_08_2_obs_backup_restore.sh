...
prepare() {
    ...
    export OBS_ACCESS=obs-access-$SUFFIX
    export OBTENANT_BP_OBS=obtenant-bp-obs-$SUFFIX
    export OBS_BACKUP_PATH=OBS-backup-path-$SUFFIX
    export OBS_ARCHIVE_PATH=OBS-archive-path-$SUFFIX
    export OBTENANT_RESTORE_OBS=obtenantrestoreobs$SUFFIX
    kubectl create namespace $NAMESPACE
    create_pass_secret $NAMESPACE $OB_ROOT_SECRET $PASSWORD
    create_pass_obs_secret $NAMESPACE $OBS_ACCESS $OBS_AK $OBS_SK 
    echo $PASSWORD
}
...
check_backup_running() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	crd_obtenantbackuppolicies_ARCHIVE=`kubectl get obtenantbackuppolicies.oceanbase.oceanbase.com -n $NAMESPACE| grep "$OBTENANT_BP_OBS"|awk -F' ' '{print $2}'| awk 'NR==1'`
	...
	if [[ $crd_obtenantbackuppolicies_ARCHIVE = "FAILED"  ]];then
	    deletefile "bp-obs.yaml"
	    sleep 5s
	    export SUFFIX=$(generate_random_str | tr '[:upper:]' '[:lower:]')
            export OBS_BACKUP_PATH=OBS-backup-path-$counter$SUFFIX
	    export OBS_ARCHIVE_PATH=OBS-archive-path-$counter$SUFFIX
	    echo $OBS_BACKUP_PATH
	    echo $OBS_ARCHIVE_PATH
	    run "bp-obs.yaml"
	fi
...
check_restore_running() {
	...
	counter=$((counter+1))
	crd_primary_restore=`kubectl get obtenant -n $NAMESPACE | grep "$OBTENANT_RESTORE_OBS"|awk -F' ' '{print $2}'`
	crd_primary_obtenant=`kubectl get obtenantrestore -n $NAMESPACE| grep "$OBTENANT_RESTORE_OBS"|awk -F' ' '{print $2}'`
...
check_restore_db_running() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	tenant_role=`mysql -h $ip -P2881 -A -uroot -p$PASSWORD -Doceanbase -e "select * from DBA_OB_TENANTS;"| grep $OBTENANT_RESTORE_OBS|awk -F' ' '{print $17}'`
	echo $tenant_role
...
validate() {
	...
	check_backup_db_running
	sleep 100s
	run "obtenant-restore-obs.yaml"
	catfile "obtenant-restore-obs.yaml"
	check_restore_running
	check_restore_db_running
