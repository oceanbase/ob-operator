...
prepare() {
    ...
    export S3_ACCESS=s3-access-$SUFFIX
    export OBTENANT_BP_S3=obtenant-bp-s3-$SUFFIX
    export S3_BACKUP_PATH=s3-backup-path-$SUFFIX
    export S3_ARCHIVE_PATH=s3-archive-path-$SUFFIX
    export OBTENANT_RESTORE_S3=obtenantrestores3$SUFFIX
    kubectl create namespace $NAMESPACE
    create_pass_secret $NAMESPACE $OB_ROOT_SECRET $PASSWORD
    create_pass_s3_secret $NAMESPACE $S3_ACCESS $S3_AK $S3_SK $S3_Region
    echo $PASSWORD
}
...
check_backup_running() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	crd_obtenantbackuppolicies_ARCHIVE=`kubectl get obtenantbackuppolicies.oceanbase.oceanbase.com -n $NAMESPACE| grep "$OBTENANT_BP_COS"|awk -F' ' '{print $2}'| awk 'NR==2'`
	...
	if [[ $crd_obtenantbackuppolicies_ARCHIVE = "FAILED"  ]];then
	    deletefile "bp-s3.yaml"
	    sleep 30s
	    export SUFFIX=$(generate_random_str | tr '[:upper:]' '[:lower:]')
            export S3_BACKUP_PATH=s3-backup-path-$counter$SUFFIX
	    export S3_ARCHIVE_PATH=s3-archive-path-$counter$SUFFIX
	    echo $S3_BACKUP_PATH $S3_ARCHIVE_PATH
	    run "bp-s3.yaml"
	    catfile "bp-s3.yaml"
	fi
...
check_restore_running() {
	...
	counter=$((counter+1))
	crd_primary_restore=`kubectl get obtenant -n $NAMESPACE | grep "$OBTENANT_RESTORE_S3"|awk -F' ' '{print $2}'`
	crd_primary_obtenant=`kubectl get obtenantrestore -n $NAMESPACE| grep "$OBTENANT_RESTORE_S3"|awk -F' ' '{print $2}'`
...
check_restore_db_running() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	tenant_role=`mysql -h $ip -P2881 -A -uroot -p$PASSWORD -Doceanbase -e "select * from DBA_OB_TENANTS;"| grep $OBTENANT_RESTORE_S3|awk -F' ' '{print $17}'`
	echo $tenant_role
...
validate() {
	...
	check_backup_db_running
	sleep 100s
	run "obtenant-restore-s3.yaml"
	catfile "obtenant-restore-s3.yaml"
	check_restore_running
	check_restore_db_running
