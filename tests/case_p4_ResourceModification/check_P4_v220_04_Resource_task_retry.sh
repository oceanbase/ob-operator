...
prepare() {
    ...
    export RESCUE_RETRY=rescue-retry-$SUFFIX
    echo $PASSWORD
}
...
check_rescue_retry_running() {
	...
	wait
	if [[ $describe_result == *"Retry Count:       0"* && $describe_result == *"Task Status:         pending"* ]]; then
            RESOURCE_RUNNING='true'
            break
        fi
        if [ $counter -eq $timeout ]; then
