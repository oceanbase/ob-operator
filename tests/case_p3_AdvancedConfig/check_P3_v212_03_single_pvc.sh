...
prepare() {
    ...
    export OBCLUSTER_NAME_PVC=testpvc$SUFFIX
...
export OBCLUSTER_NAME_PVC="$OBCLUSTER_NAME_PVC"
EOF
...
check_resource_running() {
    ...
    while true; do
        echo 'check resource'
        counter=$((counter+1))
        pod_1_zone1=`kubectl get pod  -o wide -n $NAMESPACE | grep $OBCLUSTER_NAME_PVC-1-zone1 |awk -F' ' '{print $2}'| awk 'NR==1'`
        ip=`kubectl get pod  -o wide -n $NAMESPACE | grep $OBCLUSTER_NAME_PVC-1-zone1 |awk -F' ' '{print $6}'| awk 'NR==1'`
        crd_obcluster=`kubectl get obcluster $OBCLUSTER_NAME_PVC -n $NAMESPACE  -o yaml| grep "status: running" | tail -n 1| sed 's/ //g'`
...
check_in_obcluster() {
	...
	oceanbase_DBA_OB_SERVERS_ip=`mysql -uroot -h $ip -P 2881 -p$PASSWORD  -Doceanbase  -e 'SELECT * FROM oceanbase.DBA_OB_SERVERS;'|grep $ip|awk -F' ' '{print $1}'| awk 'NR==1'` 
        echo $server_1_zone1
	echo $oceanbase_DBA_OB_SERVERS_ip
        if [[ $server_1_zone1 == "ACTIVE"  && $oceanbase_DBA_OB_SERVERS_ip = "$ip" ]]
        then
...
check_ob_pvc() {
    ...
    while true; do
	counter=$((counter+1))
	count=$(kubectl get pvc -n $NAMESPACE | grep "$OBCLUSTER_NAME_PVC-1-zone1" | wc -l)	
        if [[ "$count" -eq 1 ]]
        then
            echo "server_1_zone1 $server_1_zone1"
            OB_PVC_ACTIVE='true'
