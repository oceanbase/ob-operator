...
check_backup_mod_running() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	incrementalCrontab=`kubectl get obtenantbackuppolicies.oceanbase.oceanbase.com -n $NAMESPACE | grep $OBTENANT_NAME | awk -F' ' '{print $14$15$16$17$NF}'`
	incrementalCrontab_crd=`kubectl get obtenantbackups -n $NAMESPACE  | grep INC|awk -F' ' '{print $3}'| tail -n 1`
        if [[ $incrementalCrontab = "*/3****" && $incrementalCrontab_crd = "RUNNING" ]];then
	    echo "incrementalCrontab $incrementalCrontab incrementalCrontab_crd $incrementalCrontab_crd" 
