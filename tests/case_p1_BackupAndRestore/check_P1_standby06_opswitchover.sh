...
prepare() {
    ...
    export OP_SWITCHOVER=op-switchover$SUFFIX
}
...
cleanobtenantoperation() {
    kubectl delete obtenantoperation $OP_SWITCHOVER -n $NAMESPACE
}
...
check_obtenantoperation_running() {
    ...
    while true; do
        ...
        counter=$((counter+1))
        crd_obtenantoperation=`kubectl get obtenantoperation -n $NAMESPACE | grep "$OP_SWITCHOVER"|awk -F' ' '{print $3}'`
	...
	echo "crd_obtenantoperation $crd_obtenantoperation standby_empty_tenantrole is $standby_empty_tenantrole primary_tenantrole is $primary_tenantrole "
        if [[ $crd_obtenantoperation = "SUCCESSFUL" && $standby_empty_tenantrole = "PRIMARY" && $primary_tenantrole = "STANDBY" ]];then
