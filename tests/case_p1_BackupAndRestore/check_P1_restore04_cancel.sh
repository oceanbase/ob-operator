...
prepare() {
    export STANDBY_RESTORES=standy-restores-$SUFFIX
...
cleanrestore(){
    kubectl delete obtenant $STANDBY_RESTORES  -n $NAMESPACE
}
...
check_restore_running() {
	...
	counter=$((counter+1))
	crd_restore_obtenant=`kubectl get obtenantrestore -n $NAMESPACE| grep "$STANDBY_RESTORES"|awk -F' ' '{print $2}'`
	echo "crd_standby_restore $crd_standby_restore  crd_restore_obtenant $crd_restore_obtenant"
        if [[  $crd_restore_obtenant = "RUNNING" ]];then
...
check_cancel_restore_running() {
    counter=0
    timeout=100  
    CANCEL_RESTORE_RUNNING='false'
    ip=`kubectl get pod  -o wide -n $NAMESPACE | grep $OBCLUSTER_NAME-1-zone1 |awk -F' ' '{print $6}'| awk 'NR==1'`
    while true; do
        echo 'check RESTORE resource'
	counter=$((counter+1))
        crd_standby_restore=`kubectl get obtenant -n $NAMESPACE | grep "$STANDBY_RESTORES"|awk -F' ' '{print $2}'`
        crd_restore_obtenant=`kubectl get obtenantrestore -n $NAMESPACE| grep "$STANDBY_RESTORES"|awk -F' ' '{print $2}'`
        echo "crd_standby_restore $crd_standby_restore  crd_restore_obtenant $crd_restore_obtenant"
        if [[ $crd_standby_restore = "" && $crd_restore_obtenant = "" ]];then
            CANCEL_RESTORE_RUNNING='true'
            break
        fi
        if [ $counter -eq $timeout ]; then
...
validate() {
	...
	check_backup_db_running
	cleanrestore
	run "tenant_restore_02.yaml"
	check_restore_running
	run "tenant_restore_03.yaml"
	check_cancel_restore_running
        if [[ $CANCEL_RESTORE_RUNNING == 'false' ]]; then
            echo "case failed"
	    #cleanup
	    #prepare
        else
