...
check_backup_mod_running() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	crd_ARCHIVE_STATUS=`kubectl get obtenantbackups -n $NAMESPACE |grep ARCHIVE|awk -F' ' '{print $3}'`
        crd_policy_status=`kubectl get obtenantbackuppolicies.oceanbase.oceanbase.com -n $NAMESPACE | grep $OBTENANT_NAME | awk -F' ' '{print $2}'`
	echo "crd_ARCHIVE_STATUS $crd_ARCHIVE_STATUS crd_policy_status $crd_policy_status"
        if [[ $crd_ARCHIVE_STATUS = "RUNNING" && $crd_policy_status = "RUNNING" ]];then
	    echo "crd_ARCHIVE_STATUS $crd_ARCHIVE_STATUS crd_policy_status $crd_policy_status"
