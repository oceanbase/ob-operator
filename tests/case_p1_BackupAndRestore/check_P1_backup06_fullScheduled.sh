...
check_backup_mod_running() {
    ...
    while true; do
        ...
        counter=$((counter+1))
	fullCrontab_crd=`kubectl get obtenantbackups -n $NAMESPACE  | grep FULL|awk -F' ' '{print $3}'| tail -n 1`
	fullCrontab=`kubectl get obtenantbackuppolicies.oceanbase.oceanbase.com -n $NAMESPACE | awk -v pattern="$OBTENANT_NAME" '$0 ~ pattern {match($0, /\*\/10 \* \* \* \*/, arr); print arr[0]}'`
	echo "fullCrontab $fullCrontab fullCrontab_crd $fullCrontab_crd"
        if [[ $fullCrontab = "*/10 * * * *" && $fullCrontab_crd = "SUCCESSFUL" ]];then
