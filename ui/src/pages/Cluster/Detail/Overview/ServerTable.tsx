...
export default function ServerTable({
...
}: {clusterDetail: API.ClusterDetail[];clusterDetailRefresh: () => void;}) {
  ...
  const { runAsync: deleteOBServers } = useRequest(obcluster.deleteOBServers, {
    ...
    onSuccess: (res) => {
      if (res.successful) {
        message.success(intl.formatMessage({ id: "src.pages.Cluster.Detail.Overview.D01DC5A6", defaultMessage: "删除 Server 已成功" }));
        clusterDetailRefresh();
      }
    }
  });
  ...
  useRequest(obcluster.restartOBServers, {
    ...
    onSuccess: (res) => {
      if (res.successful) {
        message.success(intl.formatMessage({ id: "src.pages.Cluster.Detail.Overview.2A1DB242", defaultMessage: "重启 Server 已成功" }));
        clusterDetailRefresh();
      }
    }
  });
  ...
  {
    title: intl.formatMessage({ id: "src.pages.Cluster.Detail.Overview.64ED384A", defaultMessage: "操作" }),
    dataIndex: 'operation',
    render: (_, record) => {
      ...
      return (
        <>
            ...
            onClick={() => {
              Modal.confirm({
                title: intl.formatMessage({ id: "src.pages.Cluster.Detail.Overview.EFF8446F", defaultMessage: "确定要重启当前 server 吗?" }),
                onOk: () => {
            ...
            }}>{intl.formatMessage({ id: "src.pages.Cluster.Detail.Overview.E0965DEE", defaultMessage: "重启" })}
            ...
            onClick={() => {
              Modal.confirm({
                title: intl.formatMessage({ id: "src.pages.Cluster.Detail.Overview.1FE7273F", defaultMessage: "确定要删除当前 server 吗?" }),
                okType: 'danger',
                onOk: () => {
            ...
            }}>{intl.formatMessage({ id: "src.pages.Cluster.Detail.Overview.9DCE1679", defaultMessage: "删除" })}
